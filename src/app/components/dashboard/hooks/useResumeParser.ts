...
import type { ParseResumeResponse, ApiError } from '@/lib/api/types/resume.types';
...
const useResumeParser = () => {
     ...
     * @param jobRoles - Array of job roles (default: ["Data Scientist", "Data Engineer"])
     */
    const parseResume = async (
        file: File,
        jobDescription: string,
        jobRoles: string[] = ["Data Scientist", "Data Engineer"]
    ) => {
        setLoading(true);
        setError(null);
        setData(null);
        ...
        try {
            ...
            const errorMessage = apiError.message || 'An unexpected error occurred during resume parsing.';
